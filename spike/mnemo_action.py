...
Spike: only LOGS. Later, act() wakes the Mnemo agent → reconcile memory → re-score → write back.
...
class MnemoAction(Action):
    ...
    def act(self, event: EventEnvelope) -> None:
        ...
        if e.category in WAKE_ON:
            logger.info("MNEMO WAKE ▶ %s | %s/%s | %s",
                        e.entityUrn, e.category, e.operation, e.modifier)
            # DAY 7+: load prior mnemo.memory → reconcile with new graph → Bayesian re-score → write.
