...
| Eval harness (task accuracy across memory arms) | `eval/run_eval.py`, `eval/results.csv` | ✅ **BUILT & run** — controlled ablation (N=15, local Ollama, temp 0): WITHOUT 0.53 / **WITH_RAW 0.93** / WITH 1.00 / PLACEBO 0.33 (macro-F1 0.50/0.93/1.00/0.17). **WITH_RAW** strips the memory to bare key=value facts (no conclusion words) → the model *reasons* to **0.93** (lift **+0.40**), missing one subtle case — the production-realistic number, not label-parroting. PLACEBO (0.33) < WITHOUT (0.53) → the lift is *relevant* memory, not more tokens. WITH=1.00 is an acknowledged ceiling. See `examples/EVAL_NOTES.md`. |
