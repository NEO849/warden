#!/usr/bin/env python3
"""
Warden confidence model — Bayesian weight-of-evidence in log-odds.
...
AUTHORITY = {                     # a_e per evidence source
    "lineage": 1.8,
    "schema": 1.8,
    "drift_stat": 1.5,             # measured PSI/KS drift score (warden/drift.py) — below lineage/schema:
...
class Belief:
    ...
    def update(self, source: str, corroborates: bool, hops: int, quality: float,
        ...
        (e.g. "reverse-lineage" vs "static-watchlist" — see actions/warden_wake_action.py). Folded
        straight into the provenance entry (warden.provenance is what actually lands on the graph,
        see warden/memory.py::save) so callers never need a second write path or a new structured
