#!/usr/bin/env python3
"""
calibration.py — MAP weight-recovery + temperature scaling for Mnemo's confidence model (Block C).
...
(mnemo/agent.py::resolve_review → mnemo.outcome); if x_human were a feature, a fit would trivially
learn to copy its own label. freeze_features() drops any 'human' provenance entry unconditionally,
even if one were already present, and mnemo/agent.py::actuate_governance freezes x strictly BEFORE
...
def freeze_features(provenance, sources=FEATURE_SOURCES):
    ...
    on top of the caller-side guarantee (mnemo/agent.py::actuate_governance freezes x at flag time,
