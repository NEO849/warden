...
It reads only the `mnemo.*` structured properties and the `mnemo-needs-review` tag that
`mnemo/agent.py` (a separate part of this project) already writes to GMS, and renders them as a
small dark dashboard: a live heartbeat ticker, a confidence timeseries per watched model (built
from `mnemo.provenance`), and a governance queue of models currently `NEEDS_REVIEW`.
...
| var | default | meaning |
|---|---|---|
| `DATAHUB_GMS_URL` | `http://localhost:8090` | where the app reads `mnemo.*` from |
...
| `MNEMO_CONSOLE_PORT` | `8808` | port for the `python console/app.py` convenience runner (the recommended `uvicorn ... --port` flag overrides this) |
| `MNEMO_HEARTBEAT_AWAKE_WINDOW_S` | `3600` | how many seconds since the last observed wake before the heartbeat flips from "awake" to "quiet" |
...
- **Read-only.** Every route is a `GET`. No route builds or emits a
  ...
  project makes at all happen in `mnemo/agent.py`, a different file this console never imports.
...
- **GMS (`:8090`) and the DataHub UI (`:9002`) are never proxied or exposed by this app.** It
  only republishes the small `mnemo.*` fields as JSON/HTML — nothing else on the graph is
...
- `GET /` — the UI.
- `GET /api/models` — every `mlModel` Mnemo has observed: `{urn, confidence, verdict, governance_status, last_wake_ts}`.
- `GET /api/model/{urn}` — full snapshot for one URN: confidence, verdict, remembered-vs-current
  input sources, governance_status, the full `mnemo.provenance` chain, last-wake timestamp.
