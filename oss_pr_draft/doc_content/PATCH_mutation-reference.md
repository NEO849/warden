# Patch — skills/datahub-enrich/references/mutation-reference.md

INSERT the block below immediately AFTER the line `## Structured Properties` (currently line ~270
in the live repo — verify the exact line number against the current `main` before opening the PR,
upstream line numbers drift) and BEFORE the existing ```bash fenced block that starts with
`# Upsert structured property values`.
...
unchanged directly below this new block — this patch only inserts, it does not remove or reorder
anything.)
